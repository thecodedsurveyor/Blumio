...
import { useTaskActions } from "../../../hooks/stores";

const TaskAssignment = ({ task }) => {
  const { handleTaskAssignment } = useTaskActions();
  ...
  const handleAssign = async () => {
    ...
    setIsAssigning(true);
    const assignee = mockUsers.find((u) => u.id === selectedUser);

    // Simulate API delay
    setTimeout(() => {
      handleTaskAssignment(task.id, assignee);
