...
export const HamburgerButton = ({ isOpen, onClick }: HamburgerButtonProps) => (
  ...
  >
    ...
    <div className="relative h-6 w-6">
      ...
      />
      <span
        className={`absolute top-1/2 block h-0.5 w-full transform bg-current transition-opacity duration-500 ease-in-out ${
          isOpen ? "opacity-0" : "opacity-100"
        }`}
      />
      <span
        className={`absolute bottom-0 block h-0.5 w-full transform bg-current transition duration-500 ease-in-out ${
          isOpen ? "-rotate-45" : "translate-y-2"
        }`}
      />
    </div>
